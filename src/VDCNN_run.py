...
"""
VDCNN_run.py is written for run VDCNN model
"""
...
from emoji_prediction.methods.VDCNN_model import VDCNN
...
from emoji_prediction.config.VDCNN_config import LOG_PATH, TRAIN_NORMAL_DATA_PATH,\
    TEST_NORMAL_DATA_PATH, VALIDATION_NORMAL_DATA_PATH, TEXT_FIELD_PATH,\
    LABEL_FIELD_PATH, BATCH_SIZE, DEVICE, MAX_LENGTH, ONE_HOT, EMBEDDING_DIM,\
    LINEAR_UNITS, DEPTH, SHORTCUT, LOSS_CURVE_PATH, ACC_CURVE_PATH, N_EPOCHS,\
    MODEL_PATH, ADDING_NOISE, LR_DECAY
...
class RunModel:
    ...
    def init_model(data_set):
        ...
        """
        # create model
        model = VDCNN(vocab_size=data_set.num_vocab_dict["num_char"],
                      pad_idx=data_set.pad_idx_dict["token_pad_idx"],
                      n_classes=data_set.num_vocab_dict["num_label"],
                      embedding_dim=EMBEDDING_DIM, n_fc_neurons=LINEAR_UNITS,
                      depth=DEPTH, shortcut=SHORTCUT)
