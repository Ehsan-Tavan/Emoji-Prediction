...
from emoji_prediction.config.Hybrid_config import DEVICE
...
def train(model, iterator, optimizer, criterion, epoch, augmentation_class=None,
    ...
    for batch in iterator:
        n_batch += 1
        optimizer.zero_grad()
        token, token_lengths = batch.token
        char, char_lengths = batch.char
        label = batch.label

        # augment input batch
        if augmentation_class is not None:
            text, label = batch_augmentation(token, token_lengths, label,
        ...
        # predict output
        if include_length:
            predictions = model(token, token_lengths)
        else:
            predictions = model(token, char)
        # calculate loss
