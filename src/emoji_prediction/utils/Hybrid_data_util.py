...
"""
Hybrid_data_util.py is writen for creating iterator and save field
"""
...
class DataSet:
    ...
    def read_csv_file(input_path):
        ...
        input_df["chars"] = input_df["tweets"]
        return input_df[:1000]
    ...
    def create_fields(self, sen_max_len=None):
        ...
        if sen_max_len is not None:
            token_field = data.Field(tokenize=hazm.word_tokenize, batch_first=True, include_lengths=True,
                                     fix_length=sen_max_len)
            char_field = data.Field(tokenize=self.tokenizer, batch_first=True, include_lengths=True,
                                    fix_length=sen_max_len)
        else:
            token_field = data.Field(tokenize=hazm.word_tokenize, batch_first=True, include_lengths=True)
            char_field = data.Field(tokenize=self.tokenizer, batch_first=True, include_lengths=True)
    ...
    def load_data(self, token_field_path, char_field_path, label_field_path, device, batch_size,
                  sen_max_len=None, use_emotion=False, emotion_embedding_dim=10, min_freq=20,
                  one_hot=False):
        """
        load_data method is written for creating iterator for train and test data
        :param token_field_path: path for token_field
        :param char_field_path: path for char_field
        ...
        logging.info("Start creating token_field vocabs.")
        dictionary_fields["token_field"].build_vocab(train_data,
                                                     min_freq=min_freq,
        ...
        logging.info("Start creating char_field vocabs.")
        dictionary_fields["char_field"].build_vocab(train_data)

        if one_hot:
            embedding_mat = self.get_embedding_matrix(list(
                dictionary_fields["char_field"].vocab.stoi.keys()))

            dictionary_fields["char_field"].vocab.set_vectors(
                dictionary_fields["char_field"].vocab.stoi,
                torch.FloatTensor(embedding_mat),
                len(dictionary_fields["char_field"].vocab.stoi))

        # create emotion matrix
        if use_emotion:
            self.emotion_matrix = self.create_emotion_embedding(self.files_address["word_emotion_path"],
                                                                dictionary_fields["token_field"],
                                                                emotion_embedding_dim)
            logging.info("Create emotion_matrix")

        self.text_field = dictionary_fields["token_field"]
        ...
        self.embedding_dict["token_embeddings"] = \
            dictionary_fields["token_field"].vocab.vectors
        if one_hot:
            self.embedding_dict["char_embeddings"] = \
                dictionary_fields["char_field"].vocab.vectors
        ...
        # saving fields
        self.save_fields(dictionary_fields, token_field_path, char_field_path, label_field_path)
    ...
    @staticmethod
    def save_fields(dictionary_fields, token_field_path, char_field_path, label_field_path):
        """
        ...
        # save token_field
        torch.save(dictionary_fields["token_field"], token_field_path)
        logging.info("token_field is saved.")

        # save char_field
        torch.save(dictionary_fields["char_field"], char_field_path)
        logging.info("char_field is saved.")
    ...
    def calculate_num_vocabs(dictionary_fields):
        ...
        """
        num_vocab_dict = dict()
        num_vocab_dict["num_token"] = len(dictionary_fields["token_field"].vocab)
        num_vocab_dict["num_char"] = len(dictionary_fields["token_field"].vocab)
    ...
    def find_pad_index(dictionary_fields):
        ...
        """
        pad_idx_dict = dict()
        pad_idx_dict["token_pad_idx"] = dictionary_fields["token_field"]\
            .vocab.stoi[dictionary_fields["token_field"].pad_token]
        return pad_idx_dict
    ...
    def find_unk_index(dictionary_fields):
        ...
        """
        unk_idx_dict = dict()
        unk_idx_dict["token_unk_idx"] = dictionary_fields["token_field"] \
            .vocab.stoi[dictionary_fields["token_field"].unk_token]
        return unk_idx_dict
